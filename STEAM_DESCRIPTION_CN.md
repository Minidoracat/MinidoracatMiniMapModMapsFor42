[h1]🗺️ Minidoracat MiniMap - MOD Maps for B42[/h1]
[h3]By Minidoracat[/h3]

[hr][/hr]

[h2]✨ 这是什么[/h2]
[b]Minidoracat MiniMap for B42[/b] 主 MOD 的[b]地图包 addon[/b]：
收录多张地图 MOD 的小地图图像（实际游戏画面渲染）与范围框线数据。
[list]
[*] [u]自动侦测[/u]：对应的地图 MOD 有激活才显示它的图，没装不误画
[*] 地图范围框线＋名称标示（多语翻译），在小地图与世界地图上快速找到 MOD 地图
[*] 装了本包才会出现的专属选项：MOD 地图区块开关、框线开关、框线颜色（预设绿）
[*] [b]路名翻译[/b]：16 组地图数据支持繁中／简中／日文，原名与译名均可搜索
[/list]

[h2]🗺️ 收录地图与许愿[/h2]
目前开发中支持 [b]79 个地图 MOD（102 张地图）[/b]。Foxtrot Warehouse 与 Atlas Underground Complex 地表图尚未发布；[url=https://steamcommunity.com/sharedfiles/filedetails/?id=3766382352]支持地图收藏[/url]将于发布时同步。完整清单、原作者致谢、以及「许愿想要的地图」都在
[url=https://steamcommunity.com/workshop/filedetails/discussion/3763914102/568165880361411088/]讨论区[/url]——欢迎留言许愿！
也可以用 [url=https://github.com/Minidoracat/MinidoracatMiniMapModMapsFor42/issues/new/choose]GitHub Issue 表单[/url]申请支持新地图（会直接进开发待办，处理最快）。

[h2]路名翻译[/h2]
[b]16 组[/b]地图数据、360 个路名提供[b]繁中／简中／日文[/b]翻译。只翻名称，不替换原作者的道路坐标；原名与译名使用相同的实时搜索位置。[b]需要主 MOD 42.20.4-0.27.0 或更新版本[/b]，两包请一起更新并重启游戏。
未收录的街名保留原名；没有道路数据的地图不会因此获得导航路网。
街道搜索结果的「MOD 地图：地图名称」来源提示由主 MOD 提供（繁中／简中／英文／日文）；本包只提供名称对照与道路修正数据。

[h2]道路数据与支持范围[/h2]
道路修正使用独立于语言的数据：已知轮廓道路改为中心线，来源几何不符就跳过；确定重复的标签只抑制显示，不删除导航道路。不确定的重叠仍保留。图片支持不等于导航／自动驾驶认证。
详细规范与已知限制请见[url=https://steamcommunity.com/workshop/filedetails/discussion/3763914102/569297034317714585/]英中双语置顶说明[/url]。
Foxtrot Warehouse 与 Atlanta 的地块完全重叠，请二选一。Atlas Underground Complex [b]仅支持地表图，不含地下楼层[/b]，与 SecretZ 等多张地图部分重叠，不保证混用兼容。

[h2]🔧 支持范围说明[/h2]
本包提供[b]小地图／世界地图图片与街名翻译数据[/b]，不修改实际游戏地形。
[list]
[*] 图像问题（图对不上、框线位置错、名称翻译错）→ 欢迎在本页回报，或用 [url=https://github.com/Minidoracat/MinidoracatMiniMapModMapsFor42/issues/new/choose]GitHub Issue 表单[/url]（可附截屏，处理最快）
[*] 地图 MOD 本身的问题（缺材质、建筑 BUG、地图间冲突、存盘问题、[b]路名数据违规[/b]）→ 请向原地图作者回报
[/list]

[h2]🔗 系列 MOD[/h2]
[list]
[*] [b]主 MOD（必装）[/b]：[url=https://steamcommunity.com/sharedfiles/filedetails/?id=3763913359]Minidoracat MiniMap for B42[/url]——地图图片化本体
[*] [b]本页[/b]：MOD Maps——地图 MOD 图像包
[*] [b]选装[/b]：[url=https://steamcommunity.com/sharedfiles/filedetails/?id=3765182411]MOD Compatibility[/url]——第三方 MOD 兼容包（狗、马等动物图标）
[*] [b]选装[/b]：[url=https://steamcommunity.com/sharedfiles/filedetails/?id=3768276209]Zones[/url]——服务器自订区域显示
[/list]

[h2]📋 MOD 信息[/h2]
[list]
[*] [b]Mod ID:[/b] MinidoracatMiniMapModMapsFor42
[*] [b]必要 MOD:[/b] [url=https://steamcommunity.com/sharedfiles/filedetails/?id=3763913359]Minidoracat MiniMap for B42[/url]（主 MOD，[b]需 42.20.4-0.27.0 或更新版本[/b]；缺少时本包无作用）
[*] [b]加载顺序:[/b] 免手动调整——已透过 require 声明依赖，游戏自动先载主 MOD
[*] [b]支持版本:[/b] Build 42.20.0+
[*] 单机 / 多人皆可用（纯客户端图资，不改世界内容）
[/list]

[h2]💬 问题回报 & 交流[/h2]
[url=https://github.com/Minidoracat/MinidoracatMiniMapModMapsFor42/issues]🐛 GitHub Issues[/url]——地图支持申请与图像问题回报表单
[url=https://discord.gg/Gur2V67]👉 点此加入 Discord 服务器[/url]

[h2]☕ 支持作者[/h2]
MOD 永远免费。喜欢的话可以请我喝杯咖啡，赞助会用在服务器与 MOD 开发上；源代码公开在 GitHub。
[url=https://ko-fi.com/minidoracat][img]https://raw.githubusercontent.com/Minidoracat/workshop-resources/refs/heads/main/badges/badge_kofi.png[/img][/url] [url=https://github.com/Minidoracat/MinidoracatMiniMapModMapsFor42][img]https://raw.githubusercontent.com/Minidoracat/workshop-resources/refs/heads/main/badges/badge_github.png[/img][/url]

[b]#地图 #小地图 #minimap #worldmap #Minidoracat[/b]

Workshop ID: 3763914102
Mod ID: MinidoracatMiniMapModMapsFor42
