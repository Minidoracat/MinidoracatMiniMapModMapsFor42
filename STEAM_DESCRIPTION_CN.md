...
收录多张地图 MOD 的小地图图像（实际游戏画面渲染）与范围框线数据。
[list]
[*] [u]自动侦测[/u]：对应的地图 MOD 有激活才显示它的图，没装不误画
[*] 地图范围框线＋名称标示（多语翻译），在小地图与世界地图上快速找到 MOD 地图
[*] 装了本包才会出现的专属选项：MOD 地图区块开关、框线开关、框线颜色（预设绿）
[*] [b]路名翻译[/b]（0.7.0 添加）：15 张地图的路名支持繁中／简中／日文，搜索中英皆可
[/list]

[h2]🗺️ 收录地图与许愿[/h2]
目前支持 [b]69 个地图 MOD（91 张地图）[/b]，一键订阅可用 [url=https://steamcommunity.com/sharedfiles/filedetails/?id=3766382352]支持地图收藏[/url]。完整清单、原作者致谢、以及「许愿想要的地图」都在
...
[h2]🛣️ 路名翻译（0.7.0 添加）[/h2]
[b]15 张[/b]地图的路名支持[b]繁中／简中／日文[/b]（355 条），用词与官方地图一致，中英文都搜得到。需主 MOD [b]0.21.0+[/b]。
其余地图是作者原本就没做路名数据（如浣熊市），不是翻译缺漏。

[h2]⚠️ 少数地图会出现两个路名[/h2]
[b]原因在那些地图 MOD，不是本包。[/b]游戏规范是每张地图只定义自己的路，但部分作者把[b]官方路名整份拷贝或改名塞进自己的 MOD[/b]，而引擎会把两份都画出来。
本包已自动剔除重复条目（SecretZ 据点整份都是官方复本→不提供路名；马尔德劳 1993 只留自己的 52 条；雏菊郡等剔除少数条目）。
剩下路口重叠的部分要改动[b]官方[/b]路网才能消除，会影响车辆导航，故不处理——请向[b]原地图作者[/b]反映。

[h2]🔧 支持范围说明[/h2]
本包只负责[b]小地图／世界地图的图像渲染[/b]。
[list]
[*] 图像问题（图对不上、框线位置错、名称翻译错）→ 欢迎在本页回报，或用 [url=https://github.com/Minidoracat/MinidoracatMiniMapModMapsFor42/issues/new/choose]GitHub Issue 表单[/url]（可附截屏，处理最快）
...
[*] [b]必要 MOD:[/b] [url=https://steamcommunity.com/sharedfiles/filedetails/?id=3763913359]Minidoracat MiniMap for B42[/url]（主 MOD，缺少时本包无作用）
[*] [b]加载顺序:[/b] 免手动调整——已透过 require 声明依赖，游戏自动先载主 MOD
[*] [b]支持版本:[/b] Build 42.20.0+
[*] 单机 / 多人皆可用（纯客户端图资，不改世界内容）
[/list]
